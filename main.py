...
def train_model(net, criterion, optimizer, scheduler, train_loader, test_loader=None,
                print_every=100, n_epochs=10, device='cpu', save_every=1):
    for e in range(1, n_epochs+1):
        ...
        if save_every % e == 0:
            save_fname = config.SAVE_MODEL_FNAME.replace('_e_',f'_{e}_')
            torch.save(net.state_dict(), save_fname)
            print(f"Model is saved in {save_fname} after epoch: {e}")
        if test_loader != None:
...
def main():
    ...
    train_model(bert_model, criterion, optimizer, scheduler, train_loader, test_loader,
                print_every=config.PRINT_EVERY, n_epochs=config.NUM_EPOCHS,
                device=config.DEVICE, save_every=config.SAVE_EVERY)
